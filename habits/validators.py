...
class HabitValidator:
    def __call__(self, attrs):
        ...
        if associated_habit and reward:
            raise ValidationError("Нельзя одновременно заполнять 'Связанную привычку' и 'Вознаграждение'.")
        ...
        if associated_habit and not associated_habit.is_pleasant:
            raise ValidationError("Связанная привычка должна иметь признак приятной привычки (is_pleasant=True).")
